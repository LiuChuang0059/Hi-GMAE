    ...
    # heuristic for mutligrid
        ...
        for e in range(0, M):
            ...
            for kIdx in range(num_vectors):
                xk = X_jc[:, kIdx]
                proximity[e] = min(proximity[e],
                                   1 / max(np.abs(xk[i] - xk[j]) ** 2, 1e-6))  # select edges with large proximity
        ...
        for e in range(0, M):
            ...
            c[i, j] = (X_gs[i, :] @ X_gs[j, :].T) ** 2 / (
                        (X_gs[i, :] @ X_gs[i, :].T) ** 2 * (X_gs[j, :] @ X_gs[j, :].T) ** 2)  # select
    ...
    for e in range(0, M):
        ...
        # loose as little information as possible (custom)
            ...
            for kIdx in range(1, K):
                xk = X[:, kIdx]
                proximity[e] = sum(
                    [proximity[e], (xk[i] - xk[j]) ** 2])  # select edges with small proximity
        # loose as little gradient information as possible (custom)
        elif name == 'min_expected_gradient_loss':
            for kIdx in range(1, K):
                xk = X[:, kIdx]
                proximity[e] = sum([proximity[e], (xk[i] - xk[j]) ** 2 * (
                            deg[i] + deg[j] + 2 * G.W[i, j])])  # select edges with small proximity
        ...
        # relaxation ensuring that K first eigenspaces are aligned (custom)
            ...
            for kIdx in range(1, K):
                ...
                lk = G.e[kIdx]
                proximity[e] = sum([proximity[e], (xk[i] - xk[j]) ** 2 * (
                            (deg[i] + deg[j] + 2 * G.W[i, j]) / 4) / lk])  # select edges with small proximity
        ...
        # fast relaxation ensuring that K first eigenspaces are aligned (custom)
            ...
            for kIdx in range(1, K):
                ...
                lk = l_lan[kIdx]
                proximity[e] = sum([proximity[e], (xk[i] - xk[j]) ** 2 * (
                            (deg[i] + deg[j] + 2 * G.W[i, j]) / 4 - 0.5 * (
                                lk + lk)) / lk])  # select edges with small proximity
        ...
        # approximate relaxation ensuring that K first eigenspaces are aligned (custom)
            ...
            for kIdx in range(num_vectors):
                ...
                lk = xk.T @ G.L @ xk
                proximity[e] = sum([proximity[e], ((xk[i] - xk[j]) ** 2 * (
                            (deg[i] + deg[j] + 2 * G.W[i, j]) / 4 - 0 * lk) / lk)])  # select edges with small proximity
    ...
    if method == 'GS' or method == 'Gauss-Seidel':
        ...
        for j in range(num_vectors):
            ...
            for t in range(iterations):
                epsilon = 1e-6
                L_lower_diag_reg = L_lower_diag + epsilon * np.eye(L_lower_diag.shape[0])
                x = - sp.sparse.linalg.spsolve_triangular(L_lower_diag_reg, L_upper @ x)
            X[:, j] = x
        return X

    if method == 'JC' or method == 'Jacobi':

        deg = G.dw.astype(np.float64)
...
def kron_coarsening(G, r=0.5, m=None):
    ...
    try:
        Gs = my_graph_multiresolution(G, levels, r=r, sparsify=False, sparsify_eps=None, reduction_method='kron',
                                      reg_eps=0.01)
        Gk = Gs[-1]
