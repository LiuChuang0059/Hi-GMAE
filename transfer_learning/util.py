    ...
    def __call__(self, data, masked_atom_indices=None):
        ...
        * mask rate number of atom indices
        ...
        if masked_atom_indices == None:
            ...
            # num_atoms = data.x.size()[0]
            num_atoms = data.proj[-1].shape[0]
            sample_size = int(num_atoms * self.mask_rate + 1)  # mask_num
            masked_coarse_indices = random.sample(range(num_atoms), sample_size)
            # get origin mask node
            masked_atom_indices = [index if isinstance(index, int) else index for node in masked_coarse_indices
                                   for index in data.node_dict[-1][node]]
        ...
        for atom_idx in masked_atom_indices:
            new_data.x[atom_idx] = torch.tensor([self.num_atom_type, 0])
